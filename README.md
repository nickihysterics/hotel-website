...
На гостевой странице входа доступна кнопка **«Войти как демо-гость»** — она
создаёт тестовую сессию без ручного ввода и без передачи demo-пароля в браузер.
Учётные записи и пароли из таблицы предназначены только для локальной демонстрации.
...
| `DEMO_LOGIN_ENABLED` | быстрый вход тестового гостя | `1` |
| `DEMO_GUEST_EMAIL` | email аккаунта для быстрого входа | `guest@hotel.localhost` |
...
- быстрый вход работает только при явном `DEMO_LOGIN_ENABLED=1` и отключается в production;
